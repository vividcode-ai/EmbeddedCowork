...
async function fetchReleases(): Promise<Release[]> {
  ...
  while (true) {
    const url = `https://api.github.com/repos/vividcode-ai/EmbeddedCowork/releases?page=${page}&per_page=${per}`
...
async function save(githubTotal: number, npmDownloads: number) {
  ...
  try {
    execSync("npx prettier --write STATS.md", { stdio: "inherit" })
  } catch {
    console.warn("Prettier formatting skipped (not installed)")
  }
...
async function main() {
  console.log("Fetching GitHub releases for vividcode-ai/EmbeddedCowork...\n")
  ...
  console.log("Fetching npm all-time downloads for @vividcodeai/embeddedcowork...\n")
  const npmDownloads = await fetchNpmDownloads("@vividcodeai/embeddedcowork")
...
main().catch((err) => {
  console.error("Stats script failed:", err)
  process.exit(1)
})
